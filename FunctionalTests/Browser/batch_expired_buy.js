...
(async () => {
  ...
  try {
    ...
    // اضبط الكمية إلى 18 — أكبر من المتاح الصالح (17) والمنتهية 5 ستبقى غير قابلة للبيع
    await page.evaluate(() => {
    ...
    fs.writeFileSync('D:/ERPSystem/_t_expired_snacks.json', JSON.stringify(snacks), 'utf8');
  } catch (e) {
    log('FATAL=' + (e && e.message || '').slice(0, 400));
  } finally {
    fs.writeFileSync('D:/ERPSystem/_t_expired_result.txt', out.join('\n'), 'utf8');
    await browser.close();
  }
})();