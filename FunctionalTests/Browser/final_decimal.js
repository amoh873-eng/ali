...
async function main() {
  ...
  // امسح ثم اكتب فعلياً (يحاكي الكاشير الحقيقي: لوحة مفاتيح + زر النقطة)
  await input.click();
  await page.keyboard.press('Control+A');
  await page.keyboard.press('Delete');
  await input.pressSequentially('300.50', { delay: 60 });
  await page.waitForTimeout(700);
  ...
  const out = JSON.stringify({ change: ch.trim(), total: t.trim() }, null, 1);
  fs.writeFileSync('D:/ERPSystem/_final_decimal.txt', out, 'utf8');
  await browser.close();
}
main().catch(e => { fs.writeFileSync('D:/ERPSystem/_final_decimal.txt', 'FATAL ' + String(e.message).slice(0,200), 'utf8'); process.exit(1); });