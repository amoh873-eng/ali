...
{
    private async Task<GenericReportTable> GetAging(string key, DateTime from, DateTime to, DateTime asOf, Func<string,List<string>,List<List<string>>,List<string>?,string?,bool,GenericReportTable> R)
    {
        var end = to.Date;
        if(key=="ar-aging"){
            var invs=await _context.Set<ERPSystem.Domain.Entities.SalesInvoice>().Where(s=>!s.IsDeleted && s.Status!=ERPSystem.Domain.Enums.DocumentStatus.Cancelled && s.InvoiceDate>=from.Date && s.InvoiceDate<to.Date.AddDays(1)).ToListAsync();
            var rows=invs.Where(s=>s.TotalAmount - s.PaidAmount > 0.01m).Select(s=>{var bal=s.TotalAmount-s.PaidAmount; var due=(s.DueDate??s.InvoiceDate).Date; var days=(end-due).Days; string b=days<=0?"حالي":days<=30?"1-30":days<=60?"31-60":days<=90?"61-90":"90+"; return new List<string>{s.InvoiceNumber, due.ToString("yyyy-MM-dd"), days.ToString(), b, bal.ToString("N2")};}).ToList();
            return R("أعمار الذمم - عملاء",new(){ "الفاتورة","الاستحقاق","أيام","الفئة","الرصيد"},rows,null,null,true);
        } else {
            var invs=await _context.Set<ERPSystem.Domain.Entities.PurchaseInvoice>().Where(s=>!s.IsDeleted && s.Status!=ERPSystem.Domain.Enums.DocumentStatus.Cancelled && s.InvoiceDate>=from.Date && s.InvoiceDate<to.Date.AddDays(1)).ToListAsync();
            var rows=invs.Where(s=>s.TotalAmount - s.PaidAmount > 0.01m).Select(s=>{var bal=s.TotalAmount-s.PaidAmount; var due=(s.DueDate??s.InvoiceDate).Date; var days=(end-due).Days; string b=days<=0?"حالي":days<=30?"1-30":days<=60?"31-60":days<=90?"61-90":"90+"; return new List<string>{s.InvoiceNumber, due.ToString("yyyy-MM-dd"), days.ToString(), b, bal.ToString("N2")};}).ToList();
    ...
    {
        if(key=="ar-aging" || key=="ap-aging") return await GetAging(key,from,to,asOf,R);
        switch(key)
        {
            ...
            case "low-stock": { var items=await _context.Set<ERPSystem.Domain.Entities.Item>().Where(i=>!i.IsDeleted && i.CurrentStock < i.MinStockLevel).OrderBy(x=>x.Code).ToListAsync(); var rows=items.Select(i=>new List<string>{i.Code,i.NameAr,i.CurrentStock.ToString("N2"),i.MinStockLevel.ToString("N2"),(i.MinStockLevel-i.CurrentStock).ToString("N2")}).ToList(); return R("تنبيه نقص المخزون",new(){ "الرمز","الصنف","الرصيد","الحد","العجز"},rows,null,null,true); }
            case "employee-directory": { var emps=await _context.Set<ERPSystem.Domain.Entities.Employee>().Where(e=>!e.IsDeleted).Include(e=>e.Department).OrderBy(x=>x.EmployeeNumber).ToListAsync(); var rows=emps.Select(e=>new List<string>{e.EmployeeNumber,e.NameAr,e.Department?.NameAr??"",e.Status.ToString()}).ToList(); return R("دليل الموظفين",new(){ "الرقم","الاسم","القسم","الحالة"},rows,null,null,true); }
            ...
            case "pos-daily": { var invs=await _context.Set<ERPSystem.Domain.Entities.SalesInvoice>().Where(s=>s.IsPos && !s.IsDeleted && s.InvoiceDate>=from.Date && s.InvoiceDate<to.Date.AddDays(1)).ToListAsync(); var rows=new List<List<string>>{ new(){ "عدد العمليات",invs.Count.ToString()},new(){ "الإجمالي",invs.Sum(s=>s.TotalAmount).ToString("N2")}}; return R("ملخص المبيعات",new(){ "البند","القيمة"},rows,null,null,true); }
            case "leads-pipeline": { var leads=await _context.Set<ERPSystem.Domain.Entities.Lead>().Where(l=>!l.IsDeleted && (l.CreatedAt>=from.Date && l.CreatedAt<to.Date.AddDays(1))).ToListAsync(); var g=leads.GroupBy(l=>l.Status.ToString()).OrderBy(x=>x.Key).Select(x=>new List<string>{x.Key,x.Count().ToString()}).ToList(); return R("مسار العملاء المحتملين",new(){ "المرحلة","العدد"},g,null,null,true); }
