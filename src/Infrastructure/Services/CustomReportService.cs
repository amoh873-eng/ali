...
public class CustomReportService : ICustomReportService
{
    ...
    { var sql=def.SqlQuery??throw new InvalidOperationException("No SQL"); ValidateSql(sql); var connStr=_cfg.GetConnectionString("ReadOnlyConnection")??_cfg.GetConnectionString("DefaultConnection")!; using var conn=new SqlConnection(connStr); await conn.OpenAsync(ct); using var cmd=new SqlCommand(sql,conn){CommandType=CommandType.Text}; var allowed=GetAllowed(def.ParametersJson); var norm = parameters.ToDictionary(k=>k.Key, k=>k.Value, StringComparer.OrdinalIgnoreCase); foreach(var name in allowed){ object? v = null; norm.TryGetValue(name, out v); if(v is string s && string.IsNullOrWhiteSpace(s)) v=null; cmd.Parameters.AddWithValue("@"+name, v ?? DBNull.Value); } foreach(var kv in parameters) if(!allowed.Contains(kv.Key)) { } using var reader=await cmd.ExecuteReaderAsync(ct); var result=new CustomReportResult(); for(int i=0;i<reader.FieldCount;i++) result.Columns.Add(reader.GetName(i)); while(await reader.ReadAsync(ct)){var row=new Dictionary<string,object?>(); foreach(var c in result.Columns) row[c]=reader[c] is DBNull?null:reader[c]; result.Rows.Add(row);} return result; }
